...
RUN_STATUS=$(gh run list --workflow=build.yml --limit 1 --json status -q '.[0].status')
RUN_CONCLUSION=$(gh run list --workflow=build.yml --limit 1 --json conclusion -q '.[0].conclusion')
...
gh run download "$RUN_ID" --name touchfreeze-bin --dir "$TEMP_DIR"
